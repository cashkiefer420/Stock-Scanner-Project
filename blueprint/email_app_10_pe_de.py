...
JSON_FILE = os.path.join(BASE_DIR, 'json', '10_pe_de.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_pe_10_de.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_pe_1o_de.json')
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> P/E Decrease Ten Percent Notification</h1>
            <p>Dear Investor,</p>
            <p>The stock <strong>{{ stock_symbol }}</strong> has experienced a P/E movement.</p>
            <p>Current Price: ${{ current_price }}</p>
            <p>P/E Change: {{ pe }}%</p>
            <p>P/E Ratio: {{ pe_change }}%</p>
