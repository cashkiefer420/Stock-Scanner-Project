...
JSON_FILE = os.path.join(BASE_DIR, 'json', '30_mc_de.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_market_cap_30_de.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_mc_3o_de.json')
...
@app.route('/subscribe-mc-30-de', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Market Cap Decrease Thirty Percent Notification</h1>
            <p>Dear Investor,</p>
