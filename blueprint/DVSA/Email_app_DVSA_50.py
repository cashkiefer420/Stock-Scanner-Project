...
JSON_FILE = os.path.join(BASE_DIR, 'json', '50_DVSA.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_DVSA_50.json')
...
@app.route('/subscribe-DVSA-50', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> High Volume Notification !! DVSA 50% !! </h1>
            <p>Dear Investor,</p>
