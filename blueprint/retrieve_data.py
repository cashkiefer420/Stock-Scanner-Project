...
def fetch_price(ticker, pe_data, mc_data, export_data, today):
    try:
        time.sleep(0.4)  # Rate limit delay per request
        ...
        result.update({
            ...
            'DVSA (Vol Today Over Shares)': round(volume_today / shares, 4)
            ...
            'Market Cap': mc,
            'Market Cap Change (3 Mon)': calculate_percent_change(mc, current_price * shares if is_number(current_price) and is_number(shares) else 'N/A')
        })
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        results = list(executor.map(
            lambda entry: fetch_price(
                entry["Ticker"] if isinstance(entry, dict) else entry, pe_data, mc_data, export_data, today
            ), tickers))
