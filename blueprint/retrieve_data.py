...
def is_market_open():
    """Check if the market is open based on Eastern Time."""
    eastern = pytz.timezone('US/Eastern')
    now = datetime.now(eastern)
    # Market hours are 9:30 AM to 4:00 PM ET, Monday to Friday
    ...
    try:
        while not shutdown_event.is_set():
            if is_market_open():
                ...
                shutdown_event.wait(60)  # Check every minute if the market is open
