...
JSON_FILE = os.path.join(BASE_DIR, 'json', 'level_1_news.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_news_1.json')
...
@app.route('/subscribe-news-1', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> currently has one star news.</h1>
            <p>Dear Investor,</p>
            <p>The stock <strong>{{ stock_symbol }}</strong> is experiencing poor news (might be a good time to short).</p>
