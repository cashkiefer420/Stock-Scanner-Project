...
JSON_FILE = os.path.join(BASE_DIR, 'json', '75_price_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_price_75_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_price_75_in.json')
...
@app.route('/subscribe-price-75-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Increase Seventy Five Percent Notification</h1>
            <p>Dear Investor,</p>
