...
JSON_FILE = os.path.join(BASE_DIR, 'json', '15_price_de.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_price_15_de.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_price_15_de.json')
...
@app.route('/subscribe-price-15-de', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Down Fifteen Percent Notification</h1>
            <p>Dear Investor,</p>
