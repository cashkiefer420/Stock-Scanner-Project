...
JSON_FILES = {
    "1.75_volume.json": {"emails": []},
    "Filtered_volume_1.75x.json": {"stocks": []},
    "ut-1.75-volume.json": {"used_tickers": []}
}
...
# File paths
EMAILS_FILE = os.path.join(JSON_FOLDER, "1.75_volume.json")
STOCKS_FILE = os.path.join(JSON_FOLDER, "Filtered_volume_1.75x.json")
USED_TICKERS_FILE = os.path.join(JSON_FOLDER, "ut-1.75-volume.json")
...
<html>
  ...
  <body>
    <div class="email-container">
      <div class="header">
        Stock Alert: {{ stock_symbol }} 1.75 Average Volume Today
      </div>
      <div class="content">
        <p>Dear Investor,</p>
        <p>The stock <strong>{{ stock_symbol }}</strong> has High Volume today.</p>
        <p>
          <strong>Current Price:</strong> <span class="highlight">${{ current_price }}</span><br>
          <strong>Volume Today:</strong> <span class="highlight">{{ Volume_today }}%</span><br>
          <strong>Avg Volume (3 mon):</strong> <span class="highlight">${{ current_price }}</span><br>
          <strong>Average Volume:</strong> {{ AV }}
          <strong>Percentage Change:</strong> <span class="highlight">{{ DVAV }}%</span><br>
        </p>
    ...
    try:
        ...
        for stock in new_stocks:
            ...
            filled_html = template.render(
                ...
                AV=ticker_info.get("Avg Volume (3 mon)", "N/A"),
                DVAV=ticker_info.get("DVAV (Day Volume Over Average Volume)", "N/A"),
