...
JSON_FILE = os.path.join(BASE_DIR, 'json', '5x_volume.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_volume_5x.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_volume_5x.json')
...
@app.route('/subscribe-volume-5x-td', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Five Times Volume Over Average Notification</h1>
            <p>Dear Investor,</p>
