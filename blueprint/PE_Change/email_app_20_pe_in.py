...
JSON_FILE = os.path.join(BASE_DIR, 'json', '20_pe_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_pe_20_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_pe_2o_in.json')
...
@app.route('/subscribe-pe-20-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> P/E Increase Twenty Percent Notification</h1>
            <p>Dear Investor,</p>
