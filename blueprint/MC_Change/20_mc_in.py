...
JSON_FILE = os.path.join(BASE_DIR, 'json', '20_mc_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_market_cap_20_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_mc_2o_in.json')
...
@app.route('/subscribe-mc-20-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Market Cap Increase Twenty Percent Notification</h1>
            <p>Dear Investor,</p>
        ...
        for ticker_info in stock_data.get("stocks", []):
            ...
            filled_html = template.render(
                ...
                market_cap=ticker_info.get("Market Cap", "N/A"),
                percent_market_cap_change=ticker_info.get("Market Cap Change (3 Mon)", "N/A"),
            )
