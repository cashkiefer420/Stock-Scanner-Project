...
JSON_FILE = os.path.join(BASE_DIR, 'json', '30_mc_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_market_cap_30_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_mc_3o_in.json')
...
@app.route('/subscribe-mc-30-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Market Cap Increase Thirty Percent Notification</h1>
            <p>Dear Investor,</p>
