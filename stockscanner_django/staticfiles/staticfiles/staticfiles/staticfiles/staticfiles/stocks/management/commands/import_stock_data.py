...
class Command(BaseCommand):
    ...
    def fetch_with_retries(self, ticker, max_retries=3):
        for attempt in range(max_retries):
            try:
                ...
                signal.alarm(0)
                return ticker_obj, hist_data
            except Exception as e:
                signal.alarm(0)
                if "Too Many Requests" in str(e):
                    wait = 2 ** attempt
                    print(f"⚠️ Rate limit hit for {ticker}. Retrying in {wait}s...")
                    time.sleep(wait)
                else:
                    raise
        raise Exception(f"Failed after {max_retries} retries: {ticker}")

    def process_ticker(self, ticker):
        try:
            time.sleep(random.uniform(0.5, 2.0))
            ticker_obj, hist_data = self.fetch_with_retries(ticker)
            ...
            try:
                if mc and isinstance(mc, (int, float)):
                    mc_change = 0  # Placeholder, add logic if needed
                    if mc_change >= 0.30:
