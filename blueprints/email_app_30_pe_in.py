...
JSON_FILE = os.path.join(BASE_DIR, 'json', '30_pe_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_pe_30_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_pe_3o_in.json')
...
@app.route('/subscribe-pe-30-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> P/E Increase Thirty Percent Notification</h1>
            <p>Dear Investor,</p>
