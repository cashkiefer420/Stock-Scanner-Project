...
JSON_FILE = os.path.join(BASE_DIR, 'json', '10_pe_in.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_pe_10_in.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_pe_1o_in.json')
...
@app.route('/subscribe-pe-10-in', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Market Cap Increase Ten Percent Notification</h1>
            <p>Dear Investor,</p>
        ...
        for ticker_info in stock_data.get("stocks", []):
            ...
            filled_html = template.render(
                ...
                pe=ticker_info.get("Market Cap", "N/A"),
                pe_change=ticker_info.get("Market Cap Change (3 Mon)", "N/A"),
            )
