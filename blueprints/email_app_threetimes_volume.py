...
JSON_FILE = os.path.join(BASE_DIR, 'json', '3x_volume.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_volume_3x.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_volume_3x.json')
...
@app.route('/subscribe-volume-3x-td', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> Three Times Volume Over Average Notification</h1>
            <p>Dear Investor,</p>
