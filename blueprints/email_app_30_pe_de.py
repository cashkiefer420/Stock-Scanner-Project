...
JSON_FILE = os.path.join(BASE_DIR, 'json', '30_pe_de.json')
STOCK_INFO_FILE = os.path.join(BASE_DIR, 'json', 'Filtered_pe_30_de.json')
USED_TICKERS_FILE = os.path.join(BASE_DIR, 'json', 'ut_pe_3o_de.json')
...
@app.route('/subscribe-pe-30-de', methods=['POST'])
def subscribe_email():
    try:
...
def send_stock_notifications():
    try:
        ...
        <html>
          <body>
            <h1>Stock <strong>{{ stock_symbol }}</strong> P/E Decrease Thirty Percent Notification</h1>
            <p>Dear Investor,</p>
